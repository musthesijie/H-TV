...
function DoubanPageClient() {
  ...
  useEffect(() => {
    if (type === 'custom' && customCategories.length > 0) {
      ...
      if (type === 'movie') {
        ...
        // 短剧：默认走“全部”（推荐接口），避免依赖 recent_hot 的分类 type
  ...
  const getRequestParams = useCallback(
    (pageStart: number) => {
      ...
      return {
        // short / tv / show 都走 tv
        kind: (type === 'movie' ? 'movie' : 'tv') as 'tv' | 'movie',
  ...
  const loadInitialData = useCallback(async () => {
    ...
    try {
      ...
      if (type === 'custom') {
        ...
        const kind = (type === 'movie' ? 'movie' : 'tv') as 'movie' | 'tv';
        const format =
          type === 'show'
            ? '综艺'
            : type === 'tv'
      ...
      const fetchMoreData = async () => {
        ...
        try {
          ...
          if (type === 'custom') {
            ...
            const kind = (type === 'movie' ? 'movie' : 'tv') as
              | 'movie'
              | 'tv';
            const format =
              type === 'show'
